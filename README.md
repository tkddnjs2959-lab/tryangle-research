...
- [ ] `/admin` 배우 카드에 상태 뱃지와 할 일 문장이 뜨는지 · 이름/기수 검색
- [ ] `/admin/<배우>` 응답 삭제 → 집계에서 빠지는지 → **복구**되는지
- [ ] `/admin/<배우>` 변경 이력에 방금 한 일이 쌓이는지
- [ ] `/admin/inquiries` 유입 경로 집계가 뜨는지
...
### 2026-08-10 — 문의 유입(UTM) 마이그레이션 적용

홈페이지 쪽에서 만든 `20260810000001_inquiry_attribution.sql` 을 **원격 DB 에 적용**했다.
적용 전에 확인해보니 그대로 두면 **운영 중인 홈페이지가 깨지는 문제**가 있어
`drop function` 한 줄을 덧붙였다.

- 기존 `submit_inquiry` 는 인자 3개, 새 정의는 7개(기본값 4개)라
  `create or replace` 가 교체가 아니라 **오버로드**를 만든다.
- 그러면 인자 3개 호출이 두 함수 모두에 매칭되어
  `function submit_inquiry(...) is not unique` 로 실패한다.
  구버전 홈페이지가 아직 배포 중이므로 **적용하는 순간 문의 접수가 끊긴다.**
- 옛 3인자 버전을 지우면 7인자 버전이 기본값으로 그 호출을 흡수한다.
  트랜잭션으로 시험해 확인했다 — 구버전 호출은 `homepage/unknown`,
  신버전 호출은 `instagram/paid` 로 정상 저장된다.
- 덕분에 **마이그레이션과 홈페이지 배포의 순서를 맞출 필요가 없다.**
  홈페이지 쪽 코드 변경(`AttributionCapture` 등)은 아직 커밋 전이며 건드리지 않았다.

### 2026-08-10 — 배우 조회 최적화 · 검색

어드민 점검 4순위. 배우가 늘었을 때 무너질 구조를 미리 정리했다.

- `getActor(id)` 가 `listActors()` 로 **전체 배우를 읽고 그중 하나를 찾던** 구조였다.
  배우 한 명만 읽도록 바꿨다. `listCoachingStudents()` 도 전체 대신
  연동된 배우만 읽는다 (`listActorsByIds`).
- 배우 행에 진행 현황·주차·계정을 붙이는 부분을 `buildActorRows()` 하나로 모았다.
  **배우가 몇 명이든 딸린 조회는 4번**이고, 넘겨받은 배우 범위로만 좁혀 읽는다.
  세 진입점(`listActors` / `getActor` / `listActorsByIds`)이 같은 코드를 쓴다.
- `/admin` 에 이름·기수 검색 추가 (`?q=`). 목록 자체는 그대로 두고 걸러서 보여준다.
- 검증: `tsc --noEmit`, `npm run build` 통과. 원격 DB 에서 좁힌 조회가 예전
  전체 조회와 같은 집합을 주는지 확인했다.
  **화면 실물 확인은 여전히 못 했다** — 배포 후 점검 목록으로 확인이 필요하다.

### 2026-08-10 — 응답 소프트 삭제 · 변경 이력

어드민 점검 3순위. 사고가 나기 전에 되돌릴 수단을 만든다.

- 마이그레이션 `20260810000002_soft_delete_and_audit.sql` — **원격 DB 에 적용 완료**.
- 응답 삭제가 완전 삭제였다. 이제 `responses.deleted_at` 만 찍는다.
  **집계가 걸린 곳을 전부 같이 고쳤다** — `survey_progress` 뷰의 count 와
  `submit_response` 의 정원 계산. 하나라도 빠뜨리면 지운 응답이 숫자에 남아
  최소 인원이 부풀려진다.
  - `submit_response` 를 다시 만들 때 **시그니처가 원본과 같아야 한다.**
    `p_device_hash text default null` 의 기본값을 빼면
    `cannot remove parameter defaults` 로 실패한다 (한 번 겪음).
- 배우 상세의 개별 응답 목록에 삭제된 응답이 흐리게 남고 **복구** 버튼이 붙는다.
- `admin_audit_log` + `audit.ts` — 주차 공개/비공개, 배우별 예외, 응답 삭제·복구,
  코칭 연동·해제, 배우 등록을 기록한다. 배우 상세에 최근 이력을 보여준다.
  어드민 계정이 하나라 '누가' 는 아직 안 남긴다. 계정을 나누면 컬럼을 추가한다.
  **기록 실패가 본 작업을 막지 않는다** — 로그 때문에 주차 공개가 실패하면 안 된다.
- 검증: `tsc --noEmit`, `npm run build` 통과. 원격 DB 에서 응답 추가 → 2건 삭제 →
  복구를 거치며 집계가 7 → 10 → 8 → 10 으로 움직이는지 확인하고 롤백했다.

### 2026-08-10 — 알림 발송 결과 표시 · 문의 유입 표시

어드민 점검 2순위. 이미 있는 정보를 화면에 안 보여주던 것들.

- 주차를 공개하면 발송 결과(`sent`/`skipped`/`failed`)를 리다이렉트 쿼리로 넘겨
  `NotifyBanner` 로 띄운다. 그전에는 `notifyActorsWeekOpen()` 의 반환값을
  호출부에서 그냥 버려서, 대표가 몇 명에게 갔는지 알 수 없었다.
- 상담 문의 화면에 문의별 유입 태그와 **유입 경로 집계**를 추가했다.
  `inquiries.source` 는 진작 저장되고 있었는데 표시하지 않고 있었다.
- **`listInquiries()` 는 컬럼을 나열하지 않고 `select('*')` 를 쓴다.**
  홈페이지 쪽에서 준비 중인 `medium`/`campaign`/`content` 가 아직 DB 에 없어서,
  나열하면 마이그레이션 적용 전에 조회가 깨진다. 값이 `unknown` 이면 없는 것으로 친다.
- 검증: `tsc --noEmit`, `npm run build` 통과. 원격 DB 에 문의 3건을 넣어
  UTM 컬럼이 없는 상태에서도 조회와 유입 집계가 동작하는지 확인하고 롤백했다.
